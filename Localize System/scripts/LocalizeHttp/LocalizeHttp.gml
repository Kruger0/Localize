...
function LocalizeHttp() {
    ...
    if (_async[$ "id"] == _cache.requestId) {
        ...
        if (_http == 200) {
            ...
            __LocalizeTrace(LOC_TRACE.VERBOSE, _cache.traceMsg.dlGood, string_replace_all(_path, "\\", "/"));
            __LocalizeUpdate();
            ...
            if (!GM_is_sandboxed && GM_build_type == "run") {
                ...
                if (LOC_COMPRESS) {
                    var _buff = buffer_load(_pathSrc);
                ...
                __LocalizeTrace(LOC_TRACE.VERBOSE, _cache.traceMsg.fileCopy, _pathDst);
            }
        } else {
