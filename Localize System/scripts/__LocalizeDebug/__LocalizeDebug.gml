...
function __LocalizeDebug() {
    ...
    if (dbg_view_exists(__LocalizeCache().dbgvPtr)) {
        dbg_view_delete(__LocalizeCache().dbgvPtr);
    }
    ...
    __LocalizeCache().dbgvPtr = dbg_view("Localize System", _is_dbg_open, 128, 128, 320, 250);
    dbg_section("Language");
    dbg_drop_down(ref_create(__LocalizeCache(), "gameLang"), _dbgref);
    dbg_section("File");
    dbg_text($"GM is sandboxed: {GM_is_sandboxed ? "true" : "false"}");
    dbg_text("File path: " + __LocalizeCache().locPath);
