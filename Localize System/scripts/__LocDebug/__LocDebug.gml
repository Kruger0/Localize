...
function __loc_debug() {
    ...
    if (dbg_view_exists(__LocCache().dbgvPtr)) {
        dbg_view_delete(__LocCache().dbgvPtr);
    }
    ...
    __LocCache().dbgvPtr = dbg_view("Localize System", _is_dbg_open, 128, 128, 320, 250);
    dbg_section("Language");
    dbg_drop_down(ref_create(__LocCache(), "gameLang"), _dbgref);
    dbg_section("File");
    dbg_text($"GM is sandboxed: {GM_is_sandboxed ? "true" : "false"}");
    dbg_text("File path: " + __LocCache().locPath);
    dbg_text("File name: " + LOC_FILENAME);
    dbg_button("Update loc.csv online", __LocalizeInit, 300);
